...
CREATE POLICY "Allow authenticated users full access to pages" ON pages FOR ALL TO authenticated USING (true) WITH CHECK (true);
-- 2. WORKSPACES Table
ALTER TABLE workspaces ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Allow authenticated users full access to workspaces" ON workspaces;
CREATE POLICY "Allow authenticated users full access to workspaces" ON workspaces FOR ALL TO authenticated USING (true) WITH CHECK (true);
-- 3. BLOCKS Table
ALTER TABLE blocks ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Allow authenticated users full access to blocks" ON blocks;
CREATE POLICY "Allow authenticated users full access to blocks" ON blocks FOR ALL TO authenticated USING (true) WITH CHECK (true);